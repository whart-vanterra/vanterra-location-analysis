  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sorted.map(({ brand, recs, top3, topScore }) => (
          <button
            key={brand.brand_id}
            type="button"
            onClick={() => onSelectBrand(brand.brand_id)}
            className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm text-left hover:border-blue-300 hover:shadow-md transition-all"
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="font-semibold text-gray-900">{brand.display_name}</div>
                <div className="text-xs text-gray-400 font-mono">{brand.brand_id}</div>
              </div>
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${CONFIDENCE_COLORS[brand.confidence_tier] ?? ''}`}>
